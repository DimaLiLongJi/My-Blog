...
4. 开始挂载，调用 `this.$mount` 实例一个 `Watcher` 然后会调用 `this.get()` 
5. `this.get()` 会调用 `pushTarget(this)` 把当前 `Watcher` 作为第一个实例入栈
...
9. 刚才我们提到这个时候 `Dep.target` 已经被赋值为渲染 Vnode 的 `watcher` 了，那么就执行到 `addDep` 方法：
