...
当我们想在generator b中嵌套generator a时，怎么嵌套呢？
...
info.next().value.then(name => {
  console.log(name); // 测试
  const sexPromise = info.next().value;
  return sexPromise;
}).then(sex => {
  console.log(sex); // 男
  const schoolPromise = info.next().value;
...
1. `Iterator` 是一个循环接口，任何实现了此接口的数据都可以被 `for...of` 循环遍历
2. `Generator` 是一个可以暂停和继续执行的函数，可以完全实现 `Iterator` 的功能，并且由于可以保存上下文，非常适合实现简单的状态机。另外通过一些流程控制代码的配合，可以比较容易进行异步操作。
3. `Async/Await` 就是 `Generator` 进行异步操作的语法糖 或是封装了 `Promise`。返回值为 `Promise` 的函数。
